...
class OllamaClient {
    constructor() {
      ...
      console.log("Novel Dialogue Enhancer: Ollama Client initialized with enhanced capabilities");
    }
      ...
      try {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          if (this.cache.has(cacheKey)) {
            ...
            try {
              enhancedChunk = await this.processChunkWithLLM(settings.modelName, prompt, settings);
              // Cache the result
  ...
  ${chunk}`;
    ...
    async checkOllamaAvailability() {
      ...
      if (this.lastAvailabilityCheck && 
        ...
        console.log("Using cached Ollama availability result:", this.availabilityCache);
      ...
      if (this.checkingAvailability) {
        ...
        return this.availabilityCache || { available: false, reason: "Check failed" };
      }
