...
class OllamaClient {
    constructor() {
        ...
        this.CLIENT_TIMEOUT = 120000; // 2 minutes per chunk
        this.DEFAULT_CHUNK_SIZE = 8000; // Larger chunks for better context
        this.BATCH_DELAY = 800; // Delay between batches
    }
    ...
    async enhanceWithLLM(text) {
        ...
        try {
            ...
            const { maxChunkSize = this.DEFAULT_CHUNK_SIZE } = await new Promise(resolve =>
                chrome.storage.sync.get({ maxChunkSize: this.DEFAULT_CHUNK_SIZE }, resolve)
            );
...
${chunk}`;
    ...
    async processChunkWithLLM(model, prompt) {
        ...
        const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error(`LLM request timed out after ${this.CLIENT_TIMEOUT / 1000} seconds`)), this.CLIENT_TIMEOUT);
        });
