...
extractCharacterName(text) {
  ...
  if (text.length > 50) {
    ...
    for (const pattern of namePatterns) {
      ...
      if (match) {
        // Return the full match or just the captured name group
  ...
  const pronouns = ["He", "She", "It", "They", "I", "You", "We", "His", "Her", "Their", "My", "Your", "Our"];
  ...
  const nonNameWords = [
    ...
    "In", "As", "But", "Or", "And", "So", "Yet", "For", "Nor", "If",
    "From", "At", "Old", "Well", "Sister"
  ];
  ...
  // Check if text is a sentence (contains a verb or ending punctuation)
  if (text.includes(' ') && 
     (/\s(is|was|are|were|have|had|do|did|can|could|will|would|should|shall|may|might|must)\s/.test(text) ||
      /[.!?]/.test(text))) {
  ...
  const titlePattern = /^(Master|Lady|Lord|Sir|Madam|Miss|Mr\.|Mrs\.|Ms\.)\s+([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)/;
   ...
   */
  isLikelyName(text) {
    return this.extractCharacterName(text) !== null;
  }
