// dialogueUtils.js
/**
 * Dialogue utils module for Novel Dialogue Enhancer
 */
class DialogueUtils {
  constructor() {
  ...
  fixDialoguePatterns(text) {
    ...
    if (processedText.length < originalText.length * 0.9) {
      console.warn(
        "DialogueUtils: Significant text reduction detected. Reverting changes."
      );
      return originalText;
    }
  ...
  fixDialogueAttribution(text) {
    ...
    text = text.replace(
      /"([^"]+)"\s*([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s*said\s+([a-z]+ly)/g,
  ...
  fixNarrativeExpressions(text) {
    const emotionPatterns = [
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+could not help but frown/g,
        to: "$1 frowned involuntarily"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+revealed a smile/g,
        to: "$1 smiled"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+showed an? (angry|happy|sad|worried) expression/g,
        to: (m, name, emotion) => {
          switch (emotion) {
            case "angry":
              return `${name} looked angry`;
            case "happy":
              return `${name} looked pleased`;
            case "sad":
              return `${name} looked downcast`;
            case "worried":
              return `${name} looked concerned`;
            default:
              return m;
          }
        }
      ...
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+had an? ([a-z]+) look on (?:his|her) face/g,
        to: "$1 looked $2"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+let out an? ([a-z]+) breath/g,
        to: (m, name, type) => {
          switch (type) {
            case "cold":
              return `${name} exhaled coldly`;
            case "long":
              return `${name} sighed deeply`;
            case "helpless":
              return `${name} sighed helplessly`;
            default:
              return `${name} let out a ${type} breath`;
          }
        }
      }
    ];
    ...
    const actionPatterns = [
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+waved (?:his|her) hand/g,
        to: "$1 waved dismissively"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+nodded (?:his|her) head/g,
        to: "$1 nodded"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+shook (?:his|her) head/g,
        to: "$1 shook their head"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+knitted (?:his|her) brows/g,
        to: "$1 frowned"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+wrinkled (?:his|her) nose/g,
        to: "$1 wrinkled their nose"
      },
      {
        from: /([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\s+went silent/g,
        to: "$1 fell silent"
      }
    ];
  ...
  fixCulturalIdioms(text) {
    const idiomPatterns = [
      ...
      {
        from: /have eyes but fail to see Mt\. Tai/gi,
        to: "fail to recognize greatness"
      },
      { from: /lose face/gi, to: "be embarrassed" },
      { from: /give face/gi, to: "show respect" },
      { from: /save face/gi, to: "preserve dignity" },
      {
        from: /doesn't know the height of heaven and the depth of earth/gi,
        to: "has no sense of propriety"
      },
      ...
      {
        from: /wearing a different pair of trousers/gi,
        to: "playing for a different team"
      },
      {
        from: /the eagle doesn't prey on the chick/gi,
        to: "I won't bully someone weaker"
      },
      { from: /smash the pot and sink the boat/gi, to: "burn all bridges" },
      { from: /turn over the sky/gi, to: "cause chaos" },
      {
        from: /how can there be such a person/gi,
        to: "how can someone be like this"
      },
      ...
      {
        from: /climbing a tree to catch fish/gi,
        to: "using the wrong approach"
      },
      {
        from: /drawing a snake and adding legs/gi,
        to: "ruining something by overdoing it"
      },
      {
        from: /a frog at the bottom of a well/gi,
        to: "someone with a limited perspective"
      },
      { from: /covering ears to steal a bell/gi, to: "fooling oneself" },
      {
        from: /after the lips are gone, the teeth are cold/gi,
        to: "realizing the value of something after it's gone"
      },
      {
        from: /pulling chestnuts out of the fire/gi,
        to: "doing the dirty work for someone else"
      },
      {
        from: /playing the lute to a cow/gi,
        to: "wasting effort on someone who doesn't appreciate it"
      },
      {
        from: /the winds and clouds are changing/gi,
        to: "the situation is changing rapidly"
      },
      {
        from: /looking at flowers from horseback/gi,
        to: "taking only a superficial look"
      },
      {
        from: /beating the grass to startle the snake/gi,
        to: "showing one's hand prematurely"
      },
      {
        from: /hanging sheep's head but selling dog meat/gi,
        to: "false advertising"
      },
      { from: /old woman losing her only son/gi, to: "extremely distressed" },
      {
        from: /breaking the wok and sinking the boat/gi,
        to: "burning one's bridges"
      },
      { from: /chicken feathers and garlic skin/gi, to: "trivial matters" },
      {
        from: /treating a headache by foot massage/gi,
        to: "treating symptoms instead of the cause"
      },
      { from: /draw a tiger but resemble a dog/gi, to: "poor imitation" },
      { from: /a stone from another mountain/gi, to: "outside perspective" },
      {
        from: /borrowing a knife to kill someone/gi,
        to: "using someone else to do your dirty work"
      },
      {
        from: /three days without being whipped, no good can come/gi,
        to: "needs strict supervision"
      },
      {
        from: /standing on tiptoes and looking into the distance/gi,
        to: "eagerly anticipating"
      },
      {
        from: /carving insects on the beams/gi,
        to: "excessive attention to detail"
      },
  ...
  fixAddressForms(dialogue) {
    dialogue = dialogue
      .replace(
        /\b(senior|junior) (brother|sister)\b/gi,
        (match, rank, relation) => {
          if (relation.toLowerCase() === "brother") {
            return rank.toLowerCase() === "senior"
              ? "big brother"
              : "little brother";
          } else {
            return rank.toLowerCase() === "senior"
      ...
      .replace(
        /\bthis (young master|master|lady|lord|prince|princess)\b/gi,
        (match, title) => {
          return `I`;
        }
      )
      ...
      dialogue = dialogue
        ...
        .replace(/\b(very|extremely|absolutely|really)\b/gi, (match) => {
          if (Math.random() < 0.3) {
            ...
            return emphasisOptions[
              Math.floor(Math.random() * emphasisOptions.length)
            ];
          }
          return match;
        });
