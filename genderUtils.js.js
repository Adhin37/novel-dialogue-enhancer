...
function checkTitlesAndHonorifics(name) {
  ...
  const maleTitles = [
    ...
    'Emperor', 'Brother', 'Uncle', 'Father', 'Grandpa', 'Grandfather', 'Boy', 'Son',
    // Common Eastern honorifics
    'Dage', 'Gege', 'Oppa', 'Hyung', 'Nii', 'Oniisan', 'Otouto'
  ];
  
  const femaleTitles = [
    'Mrs', 'Mrs.', 'Ms', 'Ms.', 'Miss', 'Lady', 'Princess', 'Queen', 'Duchess', 'Countess',
    'Baroness', 'Empress', 'Sister', 'Aunt', 'Mother', 'Mom', 'Grandma', 'Grandmother', 'Girl', 'Daughter',
    // Common Eastern honorifics
    'Jiejie', 'Meimei', 'Unni', 'Nuna', 'Nee', 'Oneesan', 'Imouto'
  ];
...
function checkNamePatterns(name) {
  ...
  const femaleEndings = ['a', 'ie', 'y', 'ey', 'i', 'elle', 'ette', 'ine', 'ell'];
...
function analyzePronounContext(name, text) {
  ...
  const nameSentenceRegex = new RegExp(`[^.!?]*\\b${escapeRegExp(name)}\\b[^.!?]*[.!?]`, 'gi');
  ...
  matches.forEach(match => {
    ...
    const followingText = text.substring(matchIndex, matchIndex + sentenceWithName.length + 200);
...
function analyzeDescriptions(name, text) {
  ...
  // Look for gendered descriptions within 10 words of the name
  const nameContext = new RegExp(`\\b${escapeRegExp(name)}\\b[^.!?]{0,60}`, 'gi');
  ...
  const maleWords = [
    ...
    'rugged', 'masculine', 'gentleman', 'fellow', 'stocky', 'paternal'
  ];
  
  // Female-associated words
  const femaleWords = [
    'beautiful', 'pretty', 'gorgeous', 'lovely', 'she was pregnant', 'her makeup',
    'slender', 'feminine', 'graceful', 'vol
...
function escapeRegExp(string) {
  return string.replace(/[.*+?^${}()|[\]\\]/g, '\\  return { maleScore, femaleScore };');
}
...
function analyzeDescriptions(name, text) {
  ...
  const femaleWords = [
    'beautiful', 'pretty', 'gorgeous', 'lovely', 'she was pregnant', 'her makeup',
    'slender', 'feminine', 'graceful', 'voluptuous', 'maternal', 'lady', 'slim',
    'elegant', 'petite', 'curvy', 'her dress', 'her gown', 'her hair'
  ];